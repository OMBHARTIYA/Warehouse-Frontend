export default function TasksHeader({ total, visible, completed, draft }: { total: number; visible: number; completed: number; draft: number; }) {
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-[var(--border-soft)] bg-[var(--surface-2)] p-5 shadow-sm transition-colors dark:shadow-black/20 sm:p-6">
      <div>
        <h2 className="text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-4xl">Movements</h2>
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-300 sm:text-base">Track receiving, transfers, dispatches, and operational workload across warehouses.</p>
      </div>
