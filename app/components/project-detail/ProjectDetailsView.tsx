...
export default function ProjectDetailsView({ projectId }: ProjectDetailsViewProps) {
  ...
  return <section className="space-y-4"><div className="space-y-3"><h2 className="text-2xl font-semibold tracking-tight text-zinc-900 sm:text-3xl">Project Details</h2><div className="inline-flex items-center rounded-lg border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-medium text-zinc-600">Project ID: <span className="ml-1 font-semibold text-zinc-700">{projectId}</span></div></div>{isProjectLoading && <p className="text-zinc-600">Loading project details...</p>}{!isProjectLoading && projectError && <p className="text-red-600">{projectError}</p>}{!isProjectLoading && !projectError && project && <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 shadow-sm sm:p-5"><p className="text-xl font-semibold tracking-tight text-zinc-900 sm:text-2xl">{project.name?.trim() || "N/A"}</p><p className="mt-2 text-sm leading-relaxed text-zinc-600">{project.description?.trim() || "No description"}</p><div className="mt-3 flex flex-wrap items-center gap-2"><span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-medium text-zinc-700">Owner: {project.owner?.trim() || project.owner_username?.trim() || project.owner_name?.trim() || project.owner_email?.trim() || (project.owner_id != null ? `Owner #${project.owner_id}` : "N/A")}</span></div></div>}<div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm sm:p-4"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => modal.setIsCreateModalOpen(true)} className="rounded-lg bg-zinc-900 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-zinc-800">New task</button><div className="inline-flex items-center gap-1 rounded-lg border border-zinc-200 bg-zinc-50 p-1"><button type="button" onClick={() => setViewMode("board")} className={`rounded-md px-3 py-1.5 text-sm font-medium transition ${viewMode === "board" ? "bg-zinc-900 text-white" : "text-zinc-700 hover:bg-white"}`}>Board</button><button type="button" onClick={() => setViewMode("list")} className={`rounded-md px-3 py-1.5 text-sm font-medium transition ${viewMode === "list" ? "bg-zinc-900 text-white" : "text-zinc-700 hover:bg-white"}`}>List</button></div>{viewMode === "list" && <FilterDropdown id="project-task-sort" label="Sort" value={sortBy} onChange={setSortBy} className="w-52 sm:ml-auto" options={[{ value: "newest", label: "Newest first" }, { value: "oldest", label: "Oldest first" }, { value: "title", label: "Title A-Z" }, { value: "priority", label: "Priority" }]} />}</div></div>{isLoading && <LoadingSpinner label="Loading tasks..." />}{!isLoading && error && <ErrorMessage message={error} />}{!isLoading && !error && tasks.length === 0 && <p className="text-zinc-600">No tasks found.</p>}{!isLoading && !error && tasks.length > 0 && <TaskFiltersBar users={users} statusFilter={filters.statusFilter} priorityFilter={filters.priorityFilter} assigneeFilter={filters.assigneeFilter} onStatusChange={filters.setStatusFilter} onPriorityChange={filters.setPriorityFilter} onAssigneeChange={filters.setAssigneeFilter} onReset={filters.resetFilters} />}{!isLoading && !error && tasks.length > 0 && filters.filteredTasks.length === 0 && <p className="text-zinc-600">No tasks match the selected filters.</p>}{!isLoading && !error && filters.filteredTasks.length > 0 && viewMode === "list" && <TaskListView tasks={sortedListTasks} activeTaskActionId={mutations.activeTaskActionId} activeTaskActionType={mutations.activeTaskActionType} onEdit={modal.openEditModal} onDelete={setPendingDeleteTask} />}{mutations.deleteError && <p className="text-sm text-red-600">{mutations.deleteError}</p>}{!isLoading && !error && filters.filteredTasks.length > 0 && viewMode === "board" && <TaskBoardView columns={boardColumns} onDragEnd={handleBoardDragEnd} />}<TaskCreateModal isOpen={modal.isCreateModalOpen} modalRef={modal.createModalRef} titleRef={modal.createTitleInputRef} users={users} isUsersLoading={isUsersLoading} title={modal.newTaskTitle} description={modal.newTaskDescription} priority={modal.newTaskPriority} assigneeId={modal.newTaskAssigneeId} error={mutations.createError} isSubmitting={mutations.isSubmitting} onClose={modal.closeCreateModal} onSubmit={async (e) => { e.preventDefault(); const ok = await mutations.onCreate({ title: modal.newTaskTitle, description: modal.newTaskDescription, priority: modal.newTaskPriority, assigneeId: modal.newTaskAssigneeId }); if (ok) modal.closeCreateModal(); }} onTitleChange={modal.setNewTaskTitle} onDescriptionChange={modal.setNewTaskDescription} onPriorityChange={modal.setNewTaskPriority} onAssigneeChange={modal.setNewTaskAssigneeId} /><TaskEditModal isOpen={modal.isEditModalOpen} modalRef={modal.editModalRef} titleRef={modal.editTitleInputRef} users={users} isUsersLoading={isUsersLoading} title={modal.editTaskTitle} description={modal.editTaskDescription} priority={modal.editTaskPriority} status={modal.editTaskStatus} assigneeId={modal.editTaskAssigneeId} error={mutations.editError} isSaving={mutations.activeTaskActionId === modal.editingTaskId && mutations.activeTaskActionType === "edit"} onClose={modal.closeEditModal} onSubmit={async (e) => { e.preventDefault(); if (!modal.editingTaskId) return; const ok = await mutations.onEdit(modal.editingTaskId, { title: modal.editTaskTitle, description: modal.editTaskDescription, priority: modal.editTaskPriority, status: modal.editTaskStatus, assigneeId: modal.editTaskAssigneeId }); if (ok) modal.closeEditModal(); }} onTitleChange={modal.setEditTaskTitle} onDescriptionChange={modal.setEditTaskDescription} onPriorityChange={modal.setEditTaskPriority} onStatusChange={modal.setEditTaskStatus} onAssigneeChange={modal.setEditTaskAssigneeId} /><ConfirmDialog isOpen={pendingDeleteTask !== null} title="Delete task" message="Are you sure you want to delete this item? This action cannot be undone." confirmLabel="Delete task" onCancel={() => setPendingDeleteTask(null)} onConfirm={async () => { if (!pendingDeleteTask) return; await mutations.onDelete(pendingDeleteTask); setPendingDeleteTask(null); }} isBusy={mutations.activeTaskActionType === "delete"} />{mutations.lastDeletedTask && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">Task deleted. <button type="button" className="ml-2 rounded bg-emerald-700 px-2 py-1 text-xs font-medium text-white hover:bg-emerald-800" onClick={() => { void mutations.undoDeleteTask(); }}>Undo</button></div>}<Link href="/projects" className="inline-block text-sm font-medium text-zinc-900 hover:underline">Back to Projects</Link></section>;
}
