      ...
      <div ref={modalRef} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-zinc-200/80 bg-white p-5 shadow-xl ring-1 ring-zinc-200/70 sm:p-6" onClick={(event) => event.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-2.5 pt-1">
            <button type="submit" disabled={isSubmitDisabled} className="rounded-lg border border-[var(--brand-red-border)] bg-[var(--brand-red)] px-4 py-2 text-sm font-medium text-white transition hover:bg-[var(--brand-red-strong)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-red-soft)] focus-visible:ring-offset-2 disabled:opacity-60">{isSaving ? "Saving..." : "Save Changes"}</button>
