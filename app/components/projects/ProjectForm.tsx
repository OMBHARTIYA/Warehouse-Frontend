  ...
  return (
    <form onSubmit={onSubmit} className={formClassName}>
      <div className="space-y-1.5">
        <label htmlFor={nameId} className="text-sm font-medium text-zinc-700">Name</label>
        <input
          id={nameId}
          type="text"
          required
      ...
      <div className="space-y-1.5">
        <label htmlFor={descriptionId} className="text-sm font-medium text-zinc-700">Description</label>
        <textarea
          id={descriptionId}
          required
          value={description}
          onChange={(event) => onDescriptionChange(event.target.value)}
          className="min-h-28 w-full rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] px-3.5 py-2.5 text-sm text-zinc-900 outline-none transition focus:border-[var(--brand-red-border)] focus:ring-2 focus:ring-[var(--brand-red-soft)]"
        />
      </div>
