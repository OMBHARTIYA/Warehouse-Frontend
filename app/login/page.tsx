...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const typedError = error as { status?: number; payload?: { message?: string } };

      if (typedError?.status === 401) {
        setToastError(typedError.payload?.message ?? "Invalid username or password.");
      } else {
        setToastError(typedError?.payload?.message ?? "Login failed.");
      }
    } finally {
